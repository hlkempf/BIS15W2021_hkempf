...
#finds the number of NAs using sum...
```

Well, that won't work so let's remove the NAs and recalculate.
#Remember to remove NAs with `filter(!is.na`())` 
```{r}
penguins %>% 
...
#the .groups argument here just prevents a warning message
```
...
```
See 6.1 for more about `adorn` features
```{r}
penguins %>% 
  tabyl(species, island) %>% 
  adorn_percentages() %>%
...
#######using `na.rm = T` > different than !is.na in the `filter` funct
...
What if we wanted to apply summarize in order to produce distinct counts over multiple variables; i.e. species, island, and sex? Although this isn't a lot of coding you can imagine that with a lot of variables it would be cumbersome.
```{r}
penguins %>%
