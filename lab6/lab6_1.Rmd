...
## Package updates
In order to use some of the new functions in the second part of lab today, you need to update your installed R packages. Please navigate to `Tools` >`Check for Package Updates...`. Follow the directions to update the packages.  
...
```
```{r}
#get rid of NAs first, then use tabyl function, which can incorporate more than one variable
msleep %>%
  filter(!is.na(vore)) %>% 
  tabyl(order,vore)
```
...
#give name first, then tell it what that equals
```
...
```
#Don't see data for Cetaceans...
```{r}
msleep %>% 
  filter(order=="Cetacea") %>% 
  select(order,genus,brainwt)
```
That makes sense, since there are NAs there.
