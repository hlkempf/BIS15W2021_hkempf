...
2. Evaluate all numerics in `my_vector_sequence` to determine which has a value of 15.  
 Note: you can use "==" to find which one is 15 with "TRUE" or "FALSE"
...
4. If you use `[]` then you can get the values by themselves, not the logical evaluation of the entire vector. Experiment with this by adjusting the chunk below.    
```{r}
my_vector_sequence[my_vector_sequence <= 10]
```
```{r}
my_vector_sequence <= 10
#this gives you the T/F evaluation.
```
...
Create a new object called `box_office`. Here we are using the `c` command to COMBINE the vectors that we just created into one matrix.
```{r}
...
## Using a data matrix
Once you have a data matrix, you can perform lots of different analyses. For example, you can calculate the total earnings of each movie using "rowSums" and put that info into a new object.
```{r}
global <- rowSums(harry_potter_matrix)
global
```

And even add a new column to reflect this calculation. 
