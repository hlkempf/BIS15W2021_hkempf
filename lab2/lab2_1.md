...
```
## [1] "/Users/hannahkempf/Desktop/BIS15W2021_hkempf/lab2"
```
...
NAs don't have a value assigned to them, so you can't use them to do arithmetic. Because the vector we created has an NA (which has no value, and isn't one of the 5 classes of data), R can't calculate a mean for the vector. 
...
## [1] 5.888889
```
Removing the NA lets R calculate a mean, because the rest of the values are numerical and can be used to do arithmetic. the "na.rm" part of the code removes all NA's from the vector "new_vector".
