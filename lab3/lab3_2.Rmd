...
getwd()
```
Reminder that you can do `setwd()` to change directory
i.e. setwd(~/Desktop/BIS....)
...
str(hot_springs)
```
Can also use `glimpse()`...just a slightly different output.
...
```
Going to change class from `character` to `factor`. This will make all the scientists treated as the same unit. 

...characters are a string of text, where factors have levels.

```{r}
hot_springs$scientist <- as.factor(hot_springs$scientist)
class(hot_springs$scientist)
```
This gives you three levels...
...
setwd("~/Desktop/BIS15W2021_hkempf/lab3/data")
...
names(fish)
```
`name()` also used a lot to get the basic description of the data
...
```{r}
names(fish) # rememeber the names of the data
...
#load the data , i.e. the .csv file
...
Not sure why glimpse is not working...
```{r}
glimpse(mammals)
```
